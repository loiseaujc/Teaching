...
if __name__ == '__main__':
    ...
    # --> Define the function.
    f = lambda x, mu : mu*x - x**2

    # --> Define the mesh.
    x = np.linspace(-2, 2, 100)

    # --> Define the range of parameters.
    mu = np.array([-1.5, 0., 1.5])
    ...
    for i, ax in enumerate(axes):
        ...
        if mu[i] < 0:
            ...
            ax.plot(mu[i], 0, marker='o', fillstyle='none', color='royalblue', zorder=4)
            ax.plot(0, 0, 'o', color='royalblue', zorder=5)
        elif mu[i] > 0:
            ax.set_title(r'$\mu > 0$', y=-0.25)
            ax.plot(0, 0, marker='o', fillstyle='none', color='royalblue', zorder=4)
            ax.plot(mu[i], 0, 'o', color='royalblue', zorder=5)
        else:
            ax.set_title(r'$\mu = 0$', y=-0.25)
    ...
    plt.savefig('../imgs/transcritical_phase_line.pdf', bbox_inches='tight', dpi=300)
    ...
    ax.plot(x_sub, 0*x_sub, color=colors[1])
    ax.plot(x_sup, 0*x_sup, '--', color=colors[1])

    ax.plot(x_sub, x_sub, '--', color=colors[1])
    ax.plot(x_sup, x_sup, color=colors[1])
    ...
    plt.savefig('../imgs/transcritical_bifurcation_diagram.pdf', bbox_inches='tight', dpi=300)
