...
if __name__ == '__main__':
    ...
    for i, ax in enumerate(axes):
        ...
        if mu[i] < 0:
            ...
            ax.plot(-np.sqrt(mu[i]), 0, 'o', color='royalblue', zorder=6)
        else:
            ax.set_title(r'$\mu = 0$', y=-0.25)
            ax.plot(np.sqrt(mu[i]), 0, marker='o', color='royalblue', zorder=4)
    ...
    plt.savefig('../imgs/supercritical_pitchfork_phase_line.pdf', bbox_inches='tight', dpi=300)
    ...
    plt.savefig('../imgs/supercritical_pitchfork_bifurcation_diagram.pdf', bbox_inches='tight', dpi=300)
    plt.show()
