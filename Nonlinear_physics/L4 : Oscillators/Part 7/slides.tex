...
\begin{frame}[t, c]{Two-dimensional cylinder flow}{A canonical example of flow oscillators}
  ...
  \includegraphics[width=.75\textwidth]{dns_snapshot}
\end{frame}
...
\begin{frame}[t, c]{Two-dimensional cylinder flow}{Linear stability analysis}
  ...
  \begin{minipage}{.38\textwidth}
    \centering
    \begin{tikzpicture}[>=stealth]
      \filldraw[gray] (0, 0.2) circle (0.2);
      \draw[-, thick, beamer@kthblue, domain=-2:2, smooth] plot (\x , {0.2*\x*\x}) node[] {};
    \end{tikzpicture}
    ...
    \begin{tikzpicture}[>=stealth]
      \filldraw[gray] (0, 0.2) circle (0.2);
      \draw[-, thick, red, domain=-2:2, smooth] plot (\x , {-0.2*\x*\x}) node[] {};
    \end{tikzpicture}
...
\begin{frame}[t, c]{Two-dimensional cylinder flow}{Linear stability analysis}
  \begin{minipage}{.48\textwidth}
    ...
    \begin{tikzpicture}[>=stealth]
      \begin{axis}[
        ...
        \addplot[smooth, red, thick] table{data/growth_rate.dat};

        \draw[fill=gray!50, opacity=.7, draw=white] (axis cs:40, 0.7447139) -- (axis cs:45, 0.7447139) -- (axis cs:45, 1.25) -- (axis cs:40, 1.25) -- cycle ;
      \end{axis}
    \end{tikzpicture}
  \end{minipage}%
  \hfill
  \begin{minipage}{.48\textwidth}
    \begin{itemize}
    \item Leading eigenvalues come in complex-conjugate pairs.

      \bigskip

    \item \alert{\textbf{Hopf bifurcation}} at $Re \simeq 46.27$.
    \end{itemize}
  \end{minipage}
\end{frame}
...
\begin{frame}[t, c]{Two-dimensional cylinder flow}{Linear stability vs.\ real life}
  ...
  \begin{tikzpicture}[>=stealth]
    \begin{axis}[
        ...
        ylabel=Frequency ($\times 2\pi$),
        xmin=40, xmax=104,
        ymin=0.7447139, ymax=1.25,
        width=.8\textwidth,
        height=.4\textheight,
        scale only axis = true,
        legend style = {draw=none},
      ...
      \draw[fill=gray!50, opacity=.7, draw=none] (axis cs:40, 0.7447139) -- (axis cs:46.205, 0.7447139) -- (axis cs:46.205, 1.25) -- (axis cs:40, 1.25) -- cycle ;

      \addplot[smooth, red, thick] table{data/Strouhal_linear.dat};

      \only<1>{
        \legend{Linear stability}
      }

      \only<2>{
        \addplot[smooth, beamer@kthblue, thick] table{data/Strouhal_nonlinear.dat};
        \legend{Linear stability, DNS}
        }
...
\begin{frame}[t, c]{Two-dimensional cylinder flow}{Linear stability vs.\ Nonlinear evolution}
  \begin{minipage}{.48\textwidth}
    \centering
    \underline{\textbf{Stability mode}}
...
\begin{frame}[t, c]{Two-dimensional cylinder flow}{Low-dimensional representation}
  ...
  \begin{minipage}{.48\textwidth}
    ...
    \begin{tikzpicture}
      \begin{axis}[
          xmin=-1.25, ymin=-1.25, zmin=0,
          xmax=1.25, ymax=1.25, zmax=1.75,
          samples=32,
        ...
        \addplot3[red!70, thick, smooth, mark=none] table [x=x, y=y, z=z]{trajectory.txt};
      \end{axis}
    \end{tikzpicture}
  \end{minipage}%
  \hfill
  \begin{minipage}{.48\textwidth}
    ...
    \begin{tikzpicture}
      \begin{axis}[
        ...
        \addplot[red!70, thick, smooth, mark=none] table [x=x, y=y]{trajectory.txt};
      \end{axis}
    \end{tikzpicture}
  \end{minipage}%
\end{frame}
...
\begin{frame}[t, c]{Two-dimensional cylinder flow}{What insights are to be gained?}
  ...
  \begin{minipage}{.38\textwidth}
    ...
    \begin{tikzpicture}[>=stealth]
      ...
      \draw[-, thick, red, domain=0:2.8, smooth, samples=1024] plot (\x, {\x*\x/3}) node[] {};


      \node[circle, fill=white, draw=black, inner sep=0pt, minimum size=4pt] at (0, 0) {};
      \node[circle, fill=black, draw=black, inner sep=0pt, minimum size=4pt] at (2.5, 2.08333) {};
    \end{tikzpicture}
  \end{minipage}
\end{frame}
