...
# ╔═╡ 33177df6-6c9b-11ec-0779-b343e7c53e32
begin
	using Plots, OrdinaryDiffEq, PlutoUI, DiffEqJump
	plotly()
end

# ╔═╡ e03d4ef2-acd8-4d14-b5fd-317a66dc6e9f
@bind params PlutoUI.confirm(PlutoUI.combine() do Child
	md"""
# Paramètres de l'épidémie
- Taille de la population $(Child(NumberField(100:100_000_000)))
- Temps caractéristique de l'infection (semaine) $(Child(Slider(1:10, default=2, show_value=true)))
- Nombre de reproduction de base (R₀) $(Child(Slider(1:20, default=3, show_value=true)))
- Fraction de la population vaccinée $(Child(Slider(0:0.01:1, default=0, show_value=true)))
- Durée de la simulation (années) $(Child(Slider(1:0.5:10, default=1, show_value=true)))
	"""
end
)

# ╔═╡ dae129c8-92da-4897-a1d8-5356ddb53dec
begin
	# --> Transition rates
	infection_rate(u, p, t) = p[1] * u[1] * u[2]
	recovery_rate(u, p, t) = p[2] * u[2]
...
# ╔═╡ 2c0b070b-b864-410e-9acf-03e9ef72bc85
function simulate_gillepsie(N, ϵ, vax, R₀, τ, tspan)

	# --> Initial condition.
	I₀ = ceil(Int, N*ϵ)
	u₀ = [N - I₀, I₀, 0]

	# --> Convert timescale from weeks to days.
	τ = 7 * τ
	tspan = (0.0, 7*tspan[2])
	...
	jump_prob = JumpProblem(prob, Direct(), infection_jump, recovery_jump, save_positions=(false,false))
...
# ╔═╡ 08b0185d-cbba-472b-865f-2433cab96c68
...
# ╔═╡ 9e55b76f-7f76-4bd0-95eb-26ffb0a97e30
function simulate_ode(ϵ, vax, R₀, τ, tspan)
...
# ╔═╡ 3454b3cd-8b81-490b-87c6-b09adb2231a7
function plot_epidemic(ode, gil, params)

	p = plot(size=(600, 250))

	# --> ODE model.
	plot!(ode, vars=(0, 1), lw=1, color="gray", label="")
	plot!(ode, vars=(0, 2), lw=1, color="gray", label="")
	plot!(ode, vars=(0, 3), lw=1, color="gray", label="")

	# --> Stochastic model.
	t = gil.t / 7
	plot!(t, gil[1, :]/params[1], color="dodgerblue", label="S", lw=2)
	plot!(t, gil[2, :]/params[1], color="red", label="I", lw=2)
	plot!(t, gil[3, :]/params[1], color="black", label="R", lw=2)

	hline!([1/params[3]], color="black", lw=2, ls=:dash, label="")
	
	xlims!(0, 55*params[end])
	ylims!(-0.01, 1.01)

	xlabel!("Semaines")
	ylabel!("Fraction de la population")
	
	return p 
end

# ╔═╡ 6b98afea-947a-456f-bef7-ae8b8f49b735
begin
	# --> Parameters of the simulation.
	ϵ = 0.1/100
	N, τ, R₀, vax, tmax = params
	tspan = (0.0, 55*tmax)
	
	# --> Simulate epidemic (ODE model).
	epidemic_ode = simulate_ode(ϵ, vax, R₀, τ, tspan)

	# --> Simulate epidemic (SDE model).
	epidemic_gil = simulate_gillepsie(N, ϵ, vax, R₀, τ, tspan)
end;

# ╔═╡ ad7d4026-f3d6-457c-a484-e9f447cb541f
plot_epidemic(epidemic_ode, epidemic_gil, params)
