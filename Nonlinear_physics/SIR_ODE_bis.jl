...
# ╔═╡ 8d9c084a-6d5a-11ec-04fc-91bc7ebcae60
begin
	using Plots, OrdinaryDiffEq, PlutoUI
	plotly()
end

# ╔═╡ fbe58268-cb46-49bd-8089-f62660d22176
@bind params PlutoUI.confirm(PlutoUI.combine() do Child
	md"""
# Paramètres de l'épidémie
- Taille de la population (millions) $(Child(NumberField(1:100, default=70)))
- Temps caractéristique de l'infection (semaine) $(Child(Slider(1:10, default=2, show_value=true)))
- Proportion d'infectés nécessistant des soins critiques (%) $(Child(Slider(0:0.1:5, show_value=true, default=0)))
- Temps de séjour moyen en hospitalisation (semaine) $(Child(Slider(0:0.5:10, default=2, show_value=true)))
- Proportion de la population initialement vaccinée (%) $(Child(Slider(0:1:100, default=0, show_value=true)))
- Nombre de reproduction de base (R₀) $(Child(Slider(1:20, default=3, show_value=true)))
- Durée de la simulation (années) $(Child(Slider(1:10, default=1, show_value=true)))
	"""
end
)

# ╔═╡ 8f9770bf-ae3e-4ffd-bb73-93c4ce582fab
begin
	# --> Possible events
	infection(u, p, t) = p[1] * u[1] * u[2]
	recovery(u, p, t) = p[2] * u[2]
	hospitalization(u, p, t) = p[3] * u[2]
	discharge(u, p, t) = p[4] * u[3]
	...
	function model!(du, u, p, t)
		...
		# --> Model.
		du[1] = -infection(u, p, t)
		du[2] = infection(u, p, t) - recovery(u, p, t) - hospitalization(u, p, t)
		du[3] = hospitalization(u, p, t) - discharge(u, p, t)
		du[4] = recovery(u, p, t) + discharge(u, p, t)
		
		return
	end
end

# ╔═╡ a2e7c5fd-4a09-491b-833c-3b9f91a8afef
function simulate(params, I₀=0.001)

	# --> Unpack parameters.
	N, τi, α, τh, V₀, R₀, tmax = params
	#N *= 1e6
	V₀ /= 100
	α /= 100
	tmax = 55 * tmax
	
	# --> Rescale to the SIR parameters.
	γ, δ = 1/τi, 1/τh
	β = R₀ * γ / N
	α *= γ
...
# ╔═╡ 471fef5b-5217-490c-ab42-eaae71664b29
epidemie = simulate(params);

# ╔═╡ e3337e86-cbb4-4ef0-ac42-d1d6efa9ee90
function plot_epidemie(data, params)
	...
	p1 = plot!(data, vars=(0, 1), color="dodgerblue", lw=2, label="S")
	p1 = plot!(data, vars=(0, 2), color="orange", lw=2, label="I")
	p1 = plot!(data, vars=(0, 4), color="gray", lw=2, label="R")
	...
	p2 = plot!(data, vars=(0, 3), color="red", lw=2, label="H")
...
# ╔═╡ db8fd2dd-06aa-46d2-b47c-0c5bfc042543
plot_epidemie(epidemie, params)
