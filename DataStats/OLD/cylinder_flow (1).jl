...
begin
	...
	A,V=eigen(Σ)
	#rearrange the order of the vectpr
	A,V=A[end:-1:1],V[:,end:-1:1]
	# --> Plot the distribution of eigenvalues.
	plot(cumsum(A)./sum(A))
end

# ╔═╡ 8a83fd92-6d52-11eb-0b2d-cf351fa23026
DA=Diagonal(A)

# ╔═╡ 90c0d660-833e-11eb-3c70-177684ac81c1
#some of the value are less than 0 cant not be square root so picking the range of the calue is greater than zero 
DA4=DA[1:396,1:396]

# ╔═╡ bc009d60-833e-11eb-3cab-a1ff1b18b035
DA4sq=DA4^-0.5

# ╔═╡ d4f6f550-6d39-11eb-1463-770f6b0a0742
# --> Compute the PCA modes.
#the size of N is 400 
begin
	n=400
	U=(1/(n-1)^0.5)*X*V[:,1:396]*DA4sq
end
...
"
# withe the r incrasing, the size of the dataset is also increasing which make predicate of a_hat more accurate.
...
**Answer :**
"
#the higher r the less error show up so if base on this idea, it would be better to maximum the r. 
...
**Answer :**
"
# so first overeall model order recuction is a efficiecny way to simplfy the simulcation of a problem with mathemtic models. So for modeling like 1-fluid mech or like 2-air dynamics / 3-electronic dynamics are fitting a good position for this method. The biggest reason is for all those modeling there is a lot mathematic calculation and by using this mehod the calcaulting time could be reduced with a acceptable error precentage. 
